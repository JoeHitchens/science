	...
	while (<FASTQ>) {
		...
		if(exists $F_PrimerKey{$FP_seq}) {
			...
			if ($R1_seq =~ m/$probeA1{$target}|$probeA1_RC{$target}/){
				...
				if (amatch($fuzzy_A1, [ "I1", "S1", "D1" ], $R1_seq)) {
					#my $fuzzy_pos = aindex($fuzzy_A1, [ "I1", "S1", "D1" ], $R1_seq);
					my $fuzzy_str = substr $R1_seq, $TargetStart{$target}, $ProbeLength{$target};
					if (length $fuzzy_str > 14) {
						$fuzz++;
						if (exists $fuzzy_seqs{$fuzzy_str}) {$fuzzyCT{$fuzzy_str}++}
						else {$fuzzy_seqs{$fuzzy_str} = $target; $fuzzyCT{$fuzzy_str}++}
						}
				}
				elsif (amatch($fuzzy_A1RC, [ "I1", "S1", "D1" ], $R1_seq)) {
					#my $fuzzy_pos = aindex($fuzzy_A1RC, [ "I1", "S1", "D1" ], $R1_seq);
					my $fuzzy_str = substr $R1_seq, $TargetStart{$target}, $ProbeLength{$target};
				...
				elsif (amatch($fuzzy_A2, [ "I1", "S1", "D1" ], $R1_seq)) {
					#my $fuzzy_pos = aindex($fuzzy_A2, [ "I1", "S1", "D1" ], $R1_seq);
					my $fuzzy_str = substr $R1_seq, $TargetStart{$target}, $ProbeLength{$target};
					if (length $fuzzy_str > 14) {
						$fuzz++;
						if (exists $fuzzy_seqs{$fuzzy_str}) {$fuzzyCT{$fuzzy_str}++}
				...
				elsif (amatch($fuzzy_A2RC, [ "I1", "S1", "D1" ], $R1_seq)) {
					#my $fuzzy_pos = aindex($fuzzy_A1RC, [ "I1", "S1", "D1" ], $R1_seq);
					my $fuzzy_str = substr $R1_seq, $TargetStart{$target}, $ProbeLength{$target};
					$fuzzy_str = reverse $fuzzy_str;
					$fuzzy_str =~ tr/ACGT/TGCA/;
