...
#Fix allele counts to non-zero number for division ratio calculation...
	...
	$print_line{$loci} = "$loci,$allele1name{$loci}=$Allele1_Count{$loci},$allele2name{$loci}=$Allele2_Count{$loci},$ratio,$geno,$genoclass,$A1_corr{$loci},$A2_corr{$loci},$On_Target{$loci},$On_Target_Per,$Per_of_AllOTreads\n";
		}
